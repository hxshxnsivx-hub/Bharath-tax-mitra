...
def get_otp_record(mobile_number: str, table) -> Optional[Dict[str, Any]]:
    ...
    composite key: mobileNumber (PK) + timestamp (SK). (BLOCKER-5 fix)
    """
    try:
...
def verify_otp_record(mobile_number: str, otp: str, table) -> tuple[bool, str]:
...
def encrypt_mobile(mobile_number: str) -> str:
    """Encrypt mobile number using KMS (with SHA-256 hash fallback for dev)"""
    kms_key_id = os.environ.get('KMS_KEY_ID')
    if not kms_key_id:
        # Dev/staging: use SHA-256 hash (not reversible — acceptable for dev only)
        print("WARNING: KMS_KEY_ID not set. Using SHA-256 hash for mobile. Not suitable for production.")
